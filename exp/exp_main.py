        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.busy_decoder:
                    batch_y_busy = batch_y.reshape([batch_y.shape[0],-1,24]).max(-1)[0]
                # decoder input
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 10 == 0:
                    ...
                    input_busy = input[:,:,0].reshape([input.shape[0],-1,24]).max(-1)
                    gt_b = np.concatenate((input_busy[0, :], true_busy[0]), axis=0)
                    pd_b = np.concatenate((input_busy[0, :], pred_busy[0]), axis=0)
                    visual(gt_b, pd_b, os.path.join(folder_path, str(i) + '_busy.pdf'))
